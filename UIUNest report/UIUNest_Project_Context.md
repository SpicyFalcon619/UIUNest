...
## 6. Database Schema — All 15 Tables
...
| listings → applications | 1 : many | Multiple students can apply for one listing |
| users → applications | 1 : many | One student can apply to multiple listings |
...
| Section 8 | Schema Diagram + all 15 table schemas | ✅ Done |
...
11. **Applications are stored directly in the database** — rather than relying on external email/phone, a 15th table (`applications`) allows tracking inside the platform.
12. **3NF normalization throughout** — no repeating groups, no partial dependencies, no transitive dependencies
...
2. **Write SQL CREATE TABLE statements** — all 15 tables above need to be turned into MySQL-compatible `CREATE TABLE` SQL. This is what the team will use to actually build the database.
